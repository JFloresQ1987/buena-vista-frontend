...
@Component({
  selector: 'app-ahorro-pre-pago',
  templateUrl: './ahorro-pre-pago.component.html',
  styleUrls: ['./ahorro-pre-pago.component.css']
})
export class AhorroPrePagoComponent implements OnInit {
  ...
  realizarPago() {
    ...
    this.lista.forEach((item: any) => {
      ...
      const ahorro_voluntario: any = document.getElementById(item.id + '_ahorro');

      const monto_ahorro_voluntario = Number(ahorro_voluntario.value);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.serviceOperacionFinancieraPago.prePagarAhorroPorAnalista(this.listaPago)
          .subscribe(res => {
  ...
  buscarCarteraSocios(tipo: string) {
    ...
    this.tipo = this.tipos.filter(({ id }) => id === tipo)[0];
